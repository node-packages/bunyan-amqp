...
function AmqStream(options) {
  ...
  this.queue = (typeof options.queue == 'object') ? options.queue : {name: options.queue};

  if (!this.exchange.properties) {
    this.exchange.properties = {};
  }

  if (!this.queue.properties) {
  ...
  this.connection.on('ready', function () {
    self.connection.exchange(self.exchange.name, self.exchange.properties, function (exchange) {
      ...
      if (_.isUndefined(options.queue)) {
        self.connected = true;
        self.emit('connect');
        self.flush();
      } else {
        self.connection.queue(self.queue.name, self.queue.properties, function (q) {
          self._queue = q;
          ...
          q.bind(exchange, self.exchange.routingKey)
        });
      }
    });
  });
...
AmqStream.prototype.write = function (entry) {
  ...
  var overrideRoutingKey = msg.routingKey;
  delete msg.routingKey;

  this.send(overrideRoutingKey || this.exchange.routingKey || level, JSON.stringify(msg), bunyan.safeCycles());
};
