...
AmqStream.prototype.sendLog = function (routingKey, message) {
  var self = this;
  if (self._exchange) {
    var publish = self._exchange.publish(routingKey, message);
    if (self.exchange.properties.confirm) {
      publish.addListener('ack', function () {
        self.emit('ack', message);
      });
      publish.addListener('ack error', function () {
        self.emit('ack error', message);
      });
    }
  } else {
